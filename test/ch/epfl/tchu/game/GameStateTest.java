...
//class GameStateTest {
 ...
 /*   @Test
    void initial() {
        Station s1=new Station(1,"a");
        Station s2=new Station(2,"b");
        Station s3=new Station(3,"c");
        Station s4=new Station(4,"d");
        SortedBag<Ticket> tickets=SortedBag.of(1,new Ticket(s1,s2,5),1,new Ticket(s3,s4,6));
        GameState game=GameState.initial(tickets,new Random());
        System.out.println(game.tickets.size());
        System.out.println(game.cardState.deckSize());
        System.out.println(game.currentPlayerId);

        System.out.println(game.playerState.get(game.currentPlayerId).cards());
    }
    ...
    void playerState() {
        ...
        SortedBag<Ticket> tickets=SortedBag.of(1,new Ticket(s1,s2,5),1,new Ticket(s3,s4,6));
        GameState game=GameState.initial(tickets,new Random());
        System.out.println(game.playerState(game.currentPlayerId).tickets());
    ...
    void currentPlayerState() {
        ...
        SortedBag<Ticket> tickets=SortedBag.of(1,new Ticket(s1,s2,5),1,new Ticket(s3,s4,6));
        GameState game=GameState.initial(tickets,new Random());
        System.out.println(game.currentPlayerState().routes());
    ...
    void topTickets() {
        ...
        SortedBag<Ticket> tickets=SortedBag.of(1,new Ticket(s1,s2,5),1,new Ticket(s3,s4,6));
        GameState game=GameState.initial(tickets,new Random());
        assertThrows(IllegalArgumentException.class, () -> {game.topTickets(-1);
        });
    ...
    void topCard() {
        ...
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 1, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        System.out.println(game.topCard());
    }
    ...
    void topCardException() {
        ...
        System.out.println(game.cardState.deckSize());
        assertThrows(IllegalArgumentException.class, () -> {
            game.cardState.deckCard().withoutTopCards(97).topCard();
        });
    }
    ...
    void withoutTopCard() {
        ...
        GameState game = GameState.initial(tickets, new Random());
        System.out.println(game.topCard());
        System.out.println(game.withoutTopCard().cardState.deckSize());
        System.out.println(game.withoutTopCard().cardState.topDeckCard());
    }
    ...
    void withMoreDiscardedCards() {
        ...
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 1, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        SortedBag<Card> cartes=SortedBag.of(1,Card.RED,2,Card.BLACK);
        System.out.println(game.withMoreDiscardedCards(cartes).cardState.discardCards());
    }
    ...
    void withCardsDeckRecreatedIfNeeded() {
        ...
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 1, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        int k=game.cardState.deckSize();
        for(int i =0 ;i<k;i++){
          game.cardState= game.cardState.withoutTopDeckCard();
            System.out.println( game.cardState.deckSize());
        }
        SortedBag<Card>cartes=SortedBag.of(3,Card.LOCOMOTIVE,4,Card.YELLOW);
       game.cardState=game.cardState.withMoreDiscardedCards(cartes);
        System.out.println(game.cardState.discardsSize());
        System.out.println(game);
        System.out.println(game.cardState.withDeckRecreatedFromDiscards(new Random()).deckSize());
    }
    ...
    void withInitiallyChosenTickets() {
        ...
        Station s3 = new Station(3, "c");
        Station s4 = new Station(4, "d");
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 3, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        Ticket t1=new Ticket(s1, s2, 7);
        System.out.println(game.withInitiallyChosenTickets(PlayerId.PLAYER_1,SortedBag.of(2,t1)).playerState.get(PlayerId.PLAYER_1).ticketCount());
    }
    ...
    void withInitiallyChosenTicketsException() {
        ...
        GameState game = GameState.initial(tickets, new Random());
          ...
          // game.playerState.get(PlayerId.PLAYER_1)=game.playerState.get(PlayerId.PLAYER_1).withAddedTickets(SortedBag.of(1, new Ticket(s1, s2, 9), 1, new Ticket(s3, s4, 10)));
        System.out.println( game.playerState.get(PlayerId.PLAYER_2).ticketCount());
      //  assertThrows(IllegalArgumentException.class, () -> {game.withInitiallyChosenTickets(PlayerId.PLAYER_2,
             //   SortedBag.of(1, new Ticket(s1, s2, 5), 3, new Ticket(s3, s4, 6)));
      //  });
    }
    ...
    void withChosenAdditionalTickets() {
        ...
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 3, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        Ticket t1=new Ticket(s1, s2, 5);
        Ticket t2=new Ticket(s3, s4, 6);
        SortedBag<Ticket>drawntic=SortedBag.of(2, t1, 2, t2);
        SortedBag<Ticket>chosentic=SortedBag.of(1, t1, 1, t2);
        System.out.println(game.withChosenAdditionalTickets(drawntic,chosentic).playerState.get(game.currentPlayerId()).ticketCount());
        System.out.println(game.withChosenAdditionalTickets(drawntic,chosentic).tickets.size());
    }
    ...
    void withDrawnFaceUpCard() {
        ...
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 3, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        System.out.println(game.cardState.faceUpCards().toString());
        System.out.println(game.withDrawnFaceUpCard(2).cardState.faceUpCards().toString());
        System.out.println(game.withDrawnFaceUpCard(2).playerState.get(game.currentPlayerId).cards().size());
    }

    @Test
    void withDrawnFaceUpCardException() {
        Station s1 = new Station(1, "a");
        Station s2 = new Station(2, "b");
        Station s3 = new Station(3, "c");
        Station s4 = new Station(4, "d");
        ...
        GameState game = GameState.initial(tickets, new Random());
        System.out.println(game.cardState.faceUpCards().toString());
        System.out.println( game.cardState.withDrawnFaceUpCard(2).faceUpCards().toString());
       // System.out.println(game.canDrawCards());
       // assertThrows(IllegalArgumentException.class, () -> {
         //   game.withDrawnFaceUpCard(2);
        //});
    }
    ...
    void withBlindlyDrawnCard() {
        ...
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 3, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        System.out.println(game.withBlindlyDrawnCard().cardState.deckSize());
        System.out.println(game.withBlindlyDrawnCard().playerState.get(game.currentPlayerId).cards().size());
    }

    @Test
    void withBlindlyDrawnCardException() {
        Station s1 = new Station(1, "a");
        Station s2 = new Station(2, "b");
        Station s3 = new Station(3, "c");
        Station s4 = new Station(4, "d");
    ...
    void withClaimedRoute() {
        ...
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 3, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        Route route= new Route("A",s1,s2,1, Route.Level.OVERGROUND,Color.RED);
        System.out.println(game.playerState.get(game.currentPlayerId).cards().toString());
        System.out.println(game.withClaimedRoute(route,SortedBag.of(1,Card.RED)).playerState.get(game.currentPlayerId).routes().toString());
        System.out.println(game.withClaimedRoute(route,SortedBag.of(1,Card.RED)).cardState.discardCards().toString());
        System.out.println(game.withClaimedRoute(route,SortedBag.of(1,Card.RED)).playerState.get(game.currentPlayerId).cards().toString());
    }
    ...
    void lastTurnBegins() {
        ...
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 3, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        System.out.println(game.notLastTurnBegins());
        System.out.println(game.playerState.get(game.currentPlayerId).carCount());
    }
    ...
    void forNextTurn() {
        ...
        SortedBag<Ticket> tickets = SortedBag.of(1, new Ticket(s1, s2, 5), 3, new Ticket(s3, s4, 6));
        GameState game = GameState.initial(tickets, new Random());
        System.out.println(game.forNextTurn().lastPlayer);
    }*/
//}
//package ch.epfl.tchu.game;
